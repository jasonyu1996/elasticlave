...
static pte_t* __ept_walk_internal(Memory* pMemory, vaddr_t* pg_list, pte_t* root_page_table, vaddr_t addr, int create)
{
	...
	for (i = (VA_BITS - RISCV_PGSHIFT) / RISCV_PGLEVEL_BITS - 1; i > 0; i--) {
		...
		t = (pte_t *) (((vaddr_t)pte_ppn(t[idx]) << RISCV_PGSHIFT) - pMemory->start_phys_addr + (vaddr_t)root_page_table);
...
inline vaddr_t epm_va_to_pa(Memory* pMemory, pte_t* root_page_table, vaddr_t addr)
{
...
static size_t epm_alloc_vspace(Memory* pMemory, vaddr_t *pg_list, pte_t* root_page_table, vaddr_t addr, size_t num_pages)
{
