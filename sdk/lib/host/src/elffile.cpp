...
const char* ELFFile::getSectionName(size_t sh)
{
  return elf_getSectionName(&elf, sh);
}

size_t ELFFile::getNumSections(void){
	return elf_getNumSections(&elf);
}

void* ELFFile::getSection(size_t sh){
	return elf_getSection(&elf, sh);
}

size_t ELFFile::getSectionSize(size_t sh){
	return elf_getSectionSize(&elf, sh);
}

size_t ELFFile::getSectionLink(size_t sh){
	return elf_getSectionLink(&elf, sh);
}

unsigned int ELFFile::getSectionType(size_t sh){
	return (unsigned int)elf_getSectionType(&elf, sh);
}

uintptr_t ELFFile::getSectionAddr(size_t sh){
	return elf_getSectionAddr(&elf, sh);
}
