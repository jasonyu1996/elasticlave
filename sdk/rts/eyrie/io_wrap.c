...
uintptr_t io_syscall_read(int fd, void* buf, size_t len){
  ...
  if(edge_call_check_ptr_valid((uintptr_t)args->buf, len, &shared_region) != 0){
    goto done;
  }
...
uintptr_t io_syscall_openat(int dirfd, char* path,
  ...
  if(edge_call_check_ptr_valid((uintptr_t)args->path, pathlen, &shared_region) != 0){
    goto done;
  }
...
uintptr_t io_syscall_fstatat(int dirfd, char *pathname, struct stat *statbuf,
  ...
  // Sanity check that the buffer will fit in the shared memory
  if(edge_call_check_ptr_valid((uintptr_t)args->pathname, pathlen, &shared_region) != 0){
    goto done;
  }
