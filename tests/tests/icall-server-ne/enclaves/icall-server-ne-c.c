...
static char local_out_buffer[BUFFER_SIZE], local_in_buffer[BUFFER_SIZE];
...
static void icall_work_buffer(){
  memcpy(in_buf, local_in_buffer, record_size);
  icall_async((uintptr_t)server_eid, ICALL_WORK_BUFFER, NULL, 0, NULL, 0);
  memcpy(local_out_buffer, out_buf, record_size);
}
...
int main(){
  ...
  elasticlave_change(out_uid, 1);

  elasticlave_share(in_uid, server_eid, 1);
  elasticlave_share(out_uid, server_eid, 3); // exclusive read-only
